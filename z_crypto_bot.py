    ...
    try:
        ...
        while True:
            ...
            if status == 'filled':
                ...
                return False # something went wrong , May be order got cancelled manually ?
...
def get_buy_price(bids_order_book : dict,low: float ,last_price: float, my_quantity: int) -> float:
    bid_list=[]
    try:
        for bp , quantity in bids_order_book:
            bid_price = float(bp)
...
def get_sell_price(asks_order_book : dict,high: float ,last_price: float) -> float:
    ask_list=[]
    try:
        for ap , quantity in asks_order_book:
            ask_price = float(ap)
    ...
    while True:
        ...
        for market_data in res.json():
            ...
            if market_data['market'] == "ADAINR":
                ...
                bids_dict = response.json()['bids'].items()
                asks_dict=response.json()['asks'].items()
                ...
                if is_order_filled("buy",order_id,True):
                    ...
                    print_log(f"Buy order failed for {order_id}")
                    if cancel_order(order_id):
                        pass
                    else:
                        print_log(f'Buy order failed for {order_id} and could not cancel order as well')
                        status = False
